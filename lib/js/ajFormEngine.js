jQuery(document).ready(function($) {
  ...
  ajForm.get_textbox = function(field, name, secondaryName) {
    ...
    return '<input type="text" ' + ajForm.validations(field.validation) + ' ' + ajForm.attributes(field.attributes) + ' class="form-control input-sm" name="' + name + '">';
  };
    ...
    return _.each(buttonElements, function(el) {
      ...
      if (_.has(buttonObj, 'triggerClick')) {
        return $(el).bind('click', function(e) {
          var form;
          form = $(e.target).closest('form');
          return $('form').trigger("" + buttonObj.triggerClick, e);
        });
      }
    });
  };
