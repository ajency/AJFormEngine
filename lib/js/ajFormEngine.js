jQuery(document).ready(function($) {
  ...
  ajForm.addAutoSuggest = function(element) {
    ...
    _.each(divs, function(el) {
      var fieldName, item, items, magicSuggest;
  ...
  ajForm.addAutoSuggestFieldsToSerializedData = function(data) {
    var autoSuggest, divs;
    autoSuggest = ajForm.autoSuggest;
    divs = $('.ms-ctn');
    return _.each(divs, function(el, index) {
      var arr, arrSize, field, obj, selection;
      ...
      if (!$(el).is(':hidden')) {
        if (_.size(arr) === 1) {
          return data["" + arr[0]] = selection;
        } else {
          obj = data["" + arr[0]];
          return _.each(arr, function(val, i) {
            val = val.replace(']', '');
            if (i > 0) {
              if (obj["" + val] != null) {
                obj = obj["" + val];
