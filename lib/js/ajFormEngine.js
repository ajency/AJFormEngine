...
jQuery(document).ready(function($) {
  ...
  ajForm.addAutoSuggest = function(element) {
    ...
    _.each(divs, function(el) {
      ...
      magicSuggest = $(el).magicSuggest({
        ...
        ajaxConfig: {
          method: 'GET',
          headers: WP_API_NONCE ? {
            'X-WP-Nonce': WP_API_NONCE
          } : void 0
        }
      });
      $(magicSuggest).on('selectionchange', function(e, m) {
        return $(ajForm.formElement).find("input[name='" + fieldName + "']").val(JSON.stringify(this.getValue()));
      });
      ...
      return _.each(repeatableSections, function(section) {
        ...
        _.each(dataRecords, (function(_this) {
          return function(record, key) {
            ...
            sectionEle.find('input, textarea, select').each(function(index, ele) {
              var array, completeName, field_name, name, nameToReplace, property;
              ...
              if (nameToReplace) {
                ...
                return $(ele).val(record[property]);
              } else {
                return $(ele).val(key);
              }
            });
  ...
  return ajForm.updateAddressLocations = function(section, addressComponents) {
