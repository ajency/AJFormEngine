jQuery(document).ready(function($) {
  ...
  ajForm.addSection = function(evt) {
    ...
    return ajForm.triggerFieldPlugins(addedSection);
  };
  ...
  ajForm.get_radio = function(field, name, secondaryName) {
    ...
    _.each(field.options, function(option) {
      var opt;
      opt = ajForm.formatOptions(option);
      return html += '<label class="radio-inline m-l-20"> <input ' + ajForm.validations(field.validation) + ' type="radio" name="' + name + '" value="' + opt.value + '" ' + ajForm.preSelected(field, option) + '> <span class="lbl padding-8">' + opt.label + '</span> </label>';
    });
    return html;
  };
  ajForm.get_checkbox = function(field, name, secondaryName) {
    ...
    _.each(field.options, function(option) {
      var opt;
      opt = ajForm.formatOptions(option);
      return html += '<label class="radio-inline m-l-20"> <input ' + ajForm.validations(field.validation) + ' type="checkbox" name="' + name + '" value="' + opt.value + '" ' + ajForm.preSelected(field, option) + '> <span class="lbl padding-8">' + opt.label + '</span> </label>';
    });
    return html;
  };
  ...
  ajForm.triggerFieldPlugins = function(element) {
    ...
    return $(element).find('.richtext').wysihtml5();
  };
